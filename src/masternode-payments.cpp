...
void FillBlockPayee(CMutableTransaction& txNew, CAmount nFees, bool fProofOfStake, bool fZBaseStake)
{
    CBlockIndex* pindexPrev = chainActive.Tip();
    if (!pindexPrev)
        return;
    ...
    if (IsSporkActive(SPORK_13_ENABLE_SUPERBLOCKS) && budget.IsBudgetPaymentBlock(pindexPrev->nHeight + 1)) {
        ...
        masternodePayments.FillBlockPayee(txNew, nFees, fProofOfStake, fZBaseStake);
    }
}
...
void CMasternodePayments::FillBlockPayee(CMutableTransaction& txNew, int64_t nFees, bool fProofOfStake, bool fZBaseStake)
{
    ...
    if (pindexPrev->nHeight > Params().Last_PoW_Block()) {
        LogPrintf(
            ...
            "staker->payee: %s\n"
            "*******************************************************\n",
            FormatMoney(blockValue),
            FormatMoney(masternodePayment),
            GetPayeeAddress(payee),
            FormatMoney(minerStakerPayment),
            GetPayeeAddress(txNew.vout[1].scriptPubKey));
    }
...
bool CMasternodeBlockPayees::IsTransactionValid(const CTransaction& txNew)
{
    ...
    CAmount requiredMasternodePayment =
        GetMasternodePayment(nBlockHeight, nReward, nMasternode_Drift_Count, txNew.IsZerocoinSpend());
